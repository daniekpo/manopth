...
class ManoLayer(Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "th_betas", torch.Tensor(smpl_data["betas"].r).unsqueeze(0)
        )
        self.register_buffer(
            "th_shapedirs", torch.Tensor(smpl_data["shapedirs"].r)
        )
        self.register_buffer(
            "th_posedirs", torch.Tensor(smpl_data["posedirs"].r)
        )
        self.register_buffer(
            "th_v_template",
        ...
        self.register_buffer(
            "th_faces", torch.Tensor(smpl_data["f"].astype(np.int32)).long()
        )
        ...
        if self.use_pca or self.joint_rot_mode == "axisang":
            ...
            self.register_buffer(
                "th_selected_comps", torch.Tensor(selected_components)
            )
        else:
    ...
    def forward(
        ...
        if self.use_pca or self.joint_rot_mode == "axisang":
            ...
            th_hand_pose_coeffs = th_pose_coeffs[
                :, self.rot : self.rot + self.ncomps
            ]
            if self.use_pca:
        ...
        root_trans_flt = (
            root_trans.unsqueeze(1)
            .repeat(1, 5, 1, 1)
            .view(root_trans.shape[0] * 5, 4, 4)
        )
        ...
        th_results2 = (
            th_results
            - torch.cat([tmp2.new_zeros(*tmp2.shape[:2], 4, 3), tmp2], 3)
        ).permute(0, 2, 3, 1)
